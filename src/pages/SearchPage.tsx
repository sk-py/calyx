...
// Stars component from NewProducts
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8" ref={gridRef}>
            {filteredData.length > 0 ? (
              filteredData.map((perfume, index) => {
                ...
                const purchaseCount = getRandomPurchaseCount()
                const genderCategory = getGenderCategory(perfume)
                const current = perfume.price_inr ?? 45
                const original = perfume.price_inr ? Math.round(perfume.price_inr + 100) : 60
                const isNew = Boolean(perfume.isNew) || index % 4 === 1
                const reviews = purchaseCount

                return (
                  <Link to={`/product/${perfume.id}`} key={perfume.id}>
                    <div className="group hover:-translate-y-1 transition-all relative bg-white rounded-md duration-500 hover:shadow-sm font-[Doren]">
                      {/* Image */}
                      <div className="relative group/image aspect-square overflow-hidden">
                        ...
                        {/* Gender Badge - moved to top right */}
                        ...
                        {/* Hover action pill */}
                        <div className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 opacity-0 group-hover/image:opacity-100 transition-opacity duration-300">
                      ...
                      {/* Details - matching NewProducts style */}
                      <div className="p-4">
